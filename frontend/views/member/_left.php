...
<div class="card border-0">
	...
	<div class="user-menu list-group my-3">
			...
			<a href="<?=Url::toRoute(['member/contest'])?>" class="list-group-item list-group-item-action<?=Yii::$app->controller->action->id == 'contest' ? ' active' : ''?>"><i class="fa fa-users"></i> 我的组队</a>
			<a href="<?=Url::toRoute(['member/message'])?>" class="list-group-item list-group-item-action<?=Yii::$app->controller->action->id == 'message' ? ' active' : ''?>"><i class="fa fa-envelope"></i> 站内消息</a>
			<a href="<?=Url::toRoute(['member/profile'])?>" class="list-group-item list-group-item-action<?=Yii::$app->controller->action->id == 'profile' ? ' active' : ''?>"><i class="fa fa-cog"></i> 修改资料</a>
